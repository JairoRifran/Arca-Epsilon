...
export const COMBAT_VFX_QUALITY: Readonly<Record<CombatVfxQualityTier, Readonly<{
  ...
  /**
   * Sparks thrown clear of the barrel. Budgeted apart from `muzzleParticles`,
   * which dies with the flash, and never budgeted to zero: every ember in the
   * scene shares one buffer and one draw call, so the marginal cost of a spark
   * is a few floats in a loop that is already running. The other counts here
   * scale because each of their particles costs geometry; these do not.
   */
  muzzleEmbers: number;
  ...
  performance: Object.freeze({
    ...
    trailSamples: 10, muzzleParticles: 1, muzzleEmbers: 3, enemyProjectiles: 7, damageRigs: 4,
  ...
  high: Object.freeze({
    ...
    trailSamples: 14, muzzleParticles: 3, muzzleEmbers: 5, enemyProjectiles: 10, damageRigs: 6,
  ...
  ultra: Object.freeze({
    ...
    trailSamples: 20, muzzleParticles: 4, muzzleEmbers: 8, enemyProjectiles: 12, damageRigs: 12,
